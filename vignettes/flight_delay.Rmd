...
# 2. Data exploration & preparation (Steps 1–2)

First we take a look at the nycflights13 dataset via our `visualize_airport_delays()` function.

```{r}
visualize_airport_delays()
```

To anchor the magnitude of delays, we also peek at simple distribution summaries by origin airport. This helps confirm that our target has a wide spread and that each airport has enough observations for modeling.

```{r delay-summary, message=FALSE}
...
- All three airports show slightly negative medians (JFK −6min, LGA −5min, EWR −4min), so half of flights land within a few minutes of schedule or a bit early; the median differences are negligible.
- The interquartile ranges cluster around 29-32minutes, meaning the middle 50% of arrivals at each airport span roughly a half‑hour window—delays are noisy but comparably so across origins.
- EWR has the largest median (least early) and the widest IQR (32min), hinting at marginally more variability there, whereas LGA is a touch tighter (29min).
...
We rely on `caret::createDataPartition()` to obtain stratified splits that match the required proportions.
...
# caret stratified split: 80% training
train_idx <- caret::createDataPartition(dat$arr_delay, p = 0.80, list = FALSE)[, 1]
train_dat <- dat[train_idx, , drop = FALSE]

# remaining 20% -> split into validation (15%) and test (5%) via caret
remain_dat <- dat[-train_idx, , drop = FALSE]
valid_idx_rel <- caret::createDataPartition(remain_dat$arr_delay, p = 0.75, list = FALSE)[, 1]
valid_dat <- remain_dat[valid_idx_rel, , drop = FALSE]
test_dat  <- remain_dat[-valid_idx_rel, , drop = FALSE]

split_summary <- c(
  train = nrow(train_dat),
  valid = nrow(valid_dat),
  test  = nrow(test_dat)
)

# sanity check: all observations assigned exactly once
stopifnot(sum(split_summary) == N)

# proportions should be close to 0.80 / 0.15 / 0.05 (differences only from rounding)
round(split_summary / N, 3)
```
