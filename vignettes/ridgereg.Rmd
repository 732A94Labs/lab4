...
ridgereg_caret <- list(
  ...
  fit = function(x, y, wts, param, lev, last, classProbs, ...) {
    ...
    dat$.y <- y
    form <- .y ~ .
    lambda <- as.numeric(param$lambda)[1]
    lab4::ridgereg(formula = form, data = dat, lambda = lambda)
  },
  predict = function(modelFit, newdata, submodels = NULL) {
    # our predict.ridgereg already handles newdata = NULL vs data frame
...
The intercept-only baseline is clearly inadequate (RMSE about 10.4), confirming the need for predictors. Both the full linear regression and the forward-selection workflow select the same 13-variable model, giving the strongest test at RMSE ~6.18 and R-squared ~0.67. The ridge model with the tuned lambda around 5.6 trails their RMSE by less than 0.02 yet reduces MAE to roughly 3.99, showing how the shrinkage penalty dampens large residuals without hurting overall accuracy. In short, ridge regression matches the unrestricted fit while producing a more regularised coefficient profile that should generalise similarly well. For the intercept-only fit, every prediction is the same constant (the sample mean), so the predicted vector has zero variance and the correlation with the observed outcomes is undefined. Caret therefore returns NA for $R^2$; the RMSE/MAE are still computed because they only need residuals, not the correlation.
